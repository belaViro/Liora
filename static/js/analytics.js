(() => {
  ...
  const acquisitionKey='liora_acquisition';
  let acquisition;
  try { acquisition=JSON.parse(sessionStorage.getItem(acquisitionKey)||'null'); } catch (_) {}
  if(!acquisition){
    ...
    try { sessionStorage.setItem(acquisitionKey,JSON.stringify(acquisition)); } catch (_) {}
  }
