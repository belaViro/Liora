...
| 🧑‍🤝‍🧑 **多人物复盘** | v1.0.7 新增，选择最多 5 位人物围绕一段记忆进行有依据的推演 |
| 🤖 **AI 洛忆** | 懂你情绪的智能伙伴，支持悬浮入口、SSE 流式输出与第一人称视角代述 |
| 📊 **统计面板** | 全局数据画像，洞察你的记忆构成 |
| 🔮 **智能预测** | 基于图谱拓扑，预言你可能遗忘的关联 |
| 🌐 **双语切换** | 支持中文 / English UI 文案与 AI 回复语言切换 |
| 📤 **.loyi 归档** | 完整备份与迁移，包含记忆、图谱、复盘会话与待确认沉淀 |
...
**v1.0.7 新增能力**：它不是普通角色对白，而是围绕一段记忆展开的“可控、可追溯、可沉淀”的复盘工具。

- **人物选择器**：从当前记忆中的 PERSON 和相关图谱人物中选择参与者，最多 5 位，可选择是否加入“我”。
- **推演模式**：支持复盘、对质、补全空白、关系变化、如果当时等不同目标。
- **依据 / 推测标注**：每句发言都可展开查看记忆原文依据、模型推测说明与可信度，避免把猜测直接当事实。
- **结构化总结**：洛忆会归纳共识、冲突、空白和下一步问题，问题可继续用于追问推演。
- **结果沉淀**：复盘结果可保存为复盘笔记，也可提取为待确认线索或待确认关系；确认前不会写入正式图谱。

> **边界**：多人物复盘默认是探索性推演。它帮助你发现可能的解释与待确认线索，但不会把模型生成内容直接混入记忆网络。
...
`.loyi` 是 Liora 的 JSON 归档文件，当前会打包：
- `memories`：记忆原文、摘要、情感、时间与理解结果
- `entities` / `relations`：图谱实体与关系
- `agent_dialogue_sessions`：多人物复盘会话
- `agent_dialogue_candidates`：待确认复盘笔记、线索与关系

> **示例数据**：右下角「示例」按钮会加载小说《白夜》的记忆数据，并追加到现有数据中，不会清空你的本地记忆。
...
- **反向代理**：使用 Nginx / Caddy 托管 HTTPS，并确保 `text/event-stream` 不被代理缓存，保证洛忆 SSE 流式输出即时生效。
- **ICP备案**：面向中国大陆公网域名部署，已按照要求完成 ICP 备案与公安备案，并在站点页面展示双备案号。
